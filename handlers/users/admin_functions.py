...
async def send_user2_photo(message: types.Message, state: FSMContext):
    us = message.chat.id
    polz = message.from_user.username 
    print(vr[0])
    chat_id = vr[0]
    phot = message.photo[0].file_id
    get_message = f"<b>❕ Вам сообщение от Пользователя {polz}:</b>\n"
    await bot.send_photo(chat_id, photo=phot, caption=get_message)
    user_id = message.chat.id
    await bot.send_message(uchat_id, text=f"Расмотрение Обращения {polz}", reply_markup=search_profile_func(user_id))
    await message.answer("<b>Фото Успешно Отпрравленно</b>")
    vr.clear()

@dp.callback_query_handler(text_startswith="send_message_us", state="*")
async def send_user2_message(call: CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        print(call.data)
        data["here_cache_user_id"] = call.data.split(":")[1]
    ...
    await call.message.answer("<b>💌 Введите сообщение для отправки</b>\n"
                              "⚠ Сообщение будет сразу отправлено Вашему Оператору.")
    await StorageFunctions.here_send_message2.set()

@dp.message_handler(state=StorageFunctions.here_send_message2)
async def input_send_user_message(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        print(user_id)
    get_user = get_userx(user_id=user_id)
    await bot.send_message(user_id, get_message, reply_markup=search_profile_func(user_id=us))
    await message.answer("<b>Сообщение Доставленно !</b>")
    await state.finish()
